...
This vignette shows how to combine GPU monitoring from `CudaMon` with CPU and system metrics collected through `Rcollectl`.

At the moment the combined plotting helper lives in the package's experimental area, so it must be loaded explicitly.
...
`cl_start()` starts the `collectl`-based sampler, while `cm_start()` starts the NVML sampler for GPU metrics.
...
The example below records three simple workflow steps while both samplers are active.
...
Stop the GPU sampler first, then stop the `collectl` sampler and parse both outputs.
